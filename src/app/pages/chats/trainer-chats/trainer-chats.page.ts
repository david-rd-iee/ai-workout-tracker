...
@Component({
  ...
  templateUrl: './trainer-chats.page.html',
  styleUrls: ['./trainer-chats.page.scss', '../chat-shared.scss'],
  standalone: true,
  imports: [
    IonContent,
    IonAvatar,
...
export class TrainerChatsPage implements OnInit, OnDestroy {
  ...
  private initializeChats(userId: string) {
    this.chatService.initializeUserChats(userId, 'trainer');
  }
  ...
  loadChat(chatId: string, participants: string[]) {
    ...
    if (client) {
      this.router.navigate(['/chat', chatId,client, "trainer"]);
    }
  }
}